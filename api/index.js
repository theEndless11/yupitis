...
// Helpers
const { getUserFromRequest, parseBody, parseQuery, sendResponse } = require('./helpers')
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && req.url.startsWith('/api/index')) {
      const userId = query.userId || user.userId
      
      if (!userId) {
        return sendResponse(res, 400, { error: 'Missing userId parameter' })
      }
